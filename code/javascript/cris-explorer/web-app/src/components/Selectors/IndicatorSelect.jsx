...
export const IndicatorSelect = ({ dataDictionaryPath, selectedValue, setter, disabled, invalid }) => {
    ...
    const climateVariables = Object.values(data)
        .map((value) => {
            const text = t(`DataDictionary.${dataDictionaryPath}.${value}.Title`, {
                defaultValue: 'Empty',
            });
    ...
    return (
        <div className="simple-select">
            <Label htmlFor="input-select">{t(`DataDictionary.labels.indicators`)}</Label>
            {invalid ? (
                ...
                >
                    {DataDictionary.indicatorGroups.map((group, groupIdx) => {
                        return (
                            <optgroup key={`group_${groupIdx}`} label={t(`DataDictionary.indicatorGroups.${group.id}`)}>
                                {climateVariables
                                    .filter((item) => group.indicators.includes(item.value))
                                    .map((option, idx) => {
                                        return (
