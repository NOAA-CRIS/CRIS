...
export const removeAccountStorage = (manager, { key }) => {
    ...
    const active = previous.active === key ? (order.length > 0 ? order[0] : undefined) : previous.active;
...
export const beginStatusStorage = (manager, { clientId, redirectUri, portalUrl, popup }, originRoute, setActive) => {
    const previous = getLocalSerialized(manager);
    ...
    if (accounts) {
        for (const [key] of Object.entries(accounts)) {
            const deserializedSession = accounts[key].session
                ...
                : undefined;
            const deserializedPortal = accounts[key].portal ? JSON.parse(accounts[key].portal) : undefined;
